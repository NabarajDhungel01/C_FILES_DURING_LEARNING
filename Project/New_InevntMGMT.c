...
{   system("cls");// clears only one time and doesnt clear the recird that you searched for it again.
    ...
    else
    {
        ...
        fclose(filep1);
        printf("PRESS 'Y' TO SEARCH ANOTHER PRODUCT\tOR\tPress any other button to go to main menu\n\n\t\t\t\t");
    }
    repeat = getche();
    printf("\n");
...
void delete_product()
{
    ...
    if (fileptr1 == NULL) // r+ does both read and write
    {
        printf(" \n\n\t\t RECORD FILE IS EMPTY \n");
        printf("\n\t\t Enter any key to go back to main menu\n");
        getch();
    ...
    if(found == 0) // i.e. if found = 0               HERE ::  0 represents false and 1 represents true
    {
        printf("\n\nRecord not found\n");
        getch();
        display_menu();
...
void edit_product()
{
    ...
    else
    {
        ...
        scanf("%i", &target_code);
        // NOW CHECKING IF THE USER ADDED the code to edit that is previously there or not//
        // checking if the code input by user matches to the database or not.
        ///////////////////////
