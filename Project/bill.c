...
{   
    ...
    else
    {
        ...
        printf("\n\n\nEdit Product %d (Y/N) : ",target_code);
        abc = getche();
...
//textcolor(8);
    ...
    printf("SN.\tCode\tName\tClosing Stocks\tCompany\tQuantity\tRate\tTotal");
// ↑↑↑↑↑↑↑↑↑↑↑↑↑↑ TEMPLATE OF BILL ↑↑↑↑↑↑↑↑↑↑↑↑↑↑
 

 
// TO PRINT THE FRAME FOF THE BILL
    ...
    int found;
    FILE *filep1;
    filep1 = fopen("ProductDetails.dat","r+"); // we are only reading this file
    ...
    while (fscanf(filep1,"%i %s %s %i %i\n", &pdt.product_code, pdt.product_name, pdt.product_company, &pdt.product_price, &pdt.product_quantity) != EOF)
        {
            ...
            {
                gotopositionxy(40, 7);
                printf("***Found***      || Name || :- \"%s\"  || Rate || :- \"\"  ||Total Qty.|| :- \"%d\" ",pdt.product_name,pdt.product_price,pdt.product_quantity);
                    // gotopositionxy(34,7);
                    // printf("%d",code); // this will print between the enter the code of product and found.
                found = 1;
    ...
    if (found)
    {
     ...
     scanf("%d",&qt);
     FILE *filep2;
     filep2 = fopen("Temp_ProductDetails_Bills.dat","a+");

        printf("\nConfirm -- || Code -%d ||  ||Name - %s||   ||Quantity - %d|| ",code, pdt.product_name, qt);
        printf("\n(Y/N)?\t: ");
        char check;
        check = getche();
        
        // REDUCING THE QUANTITY IN THE FILE
        
        if(check=='y' || check =='Y') // if false
           {
               int total;
               total = pdt.product_price * qt;
               gotopositionxy(13,27+number_of_items);
               printf("%d\t\t%d/%d\t%s\t    %d\t%s\t\t\t%d\t%d",number_of_items, code,pdt.product_code,pdt.product_name, pdt.product_quantity,pdt.product_company,pdt.product_price,total);
               // REDUCING THE quantity in the file
               while (fscanf(filep1,"%d %s %s %i %i\n",&pdt.product_code, pdt.product_name, pdt.product_company, &pdt.product_price,&pdt.product_quantity)!=EOF)
               {
                   if (code == pdt.product_code)
                   {
                       pdt.product_quantity = pdt.product_quantity - qt; // this will reduce the number of stocks in the system
                   }
                    fprintf(filep2,"%i %s %s %i %i\n", pdt.product_code, pdt.product_name, pdt.product_company, pdt.product_price, pdt.product_quantity);
               }
                fclose(filep2);
                fclose(filep1);
                // remove("ProductDetails.dat");
                // rename("Temp_ProductDetails_Bills.dat","ProductDetails.dat");
           }   
    }
