...
void edit_product()
{
    ...
    // 
        // STORING THESE VALUES SO THAT IF THE USER PRESS no to edit, the origingal data will be printed
    int  original_temp_code = 0;
    char original_temp_name[40];
    char original_temp_company_name[40];
    int  original_temp_price = 0;
    int  original_temp_total_quantity = 0;
    //
    ...
    else
    {
        ...
        while (fscanf(filep1, "%i %s %s %i %i\n", &pdt.product_code, pdt.product_name, pdt.product_company, &pdt.product_price, &pdt.product_quantity) != EOF)
        {
            if (target_code == pdt.product_code)
            {
                ...
                // storing original values of the databse so that if the user do not agreee to edit the product at last, the original values must be printed to the database

                    original_temp_code = pdt.product_code;
                    original_temp_price = pdt.product_price;
                    original_temp_total_quantity = pdt.product_quantity;
                    strcpy(original_temp_name,pdt.product_name);
                    strcpy(original_temp_company_name,pdt.product_company);
                ...
                while (fscanf(filep1, "%i %s %s %i %i", &pdt.product_code, pdt.product_name, pdt.product_company, &pdt.product_price, &pdt.product_quantity) != EOF)
                {
                    ...
                    */
                    if (target_code == pdt.product_code)
                    {   
                        // if the user wants to give the same code number while editing the product.
                        if (pdt.product_price == original_temp_price && pdt.product_quantity == original_temp_total_quantity)
                        {
                            goto keeping_the_same_code_as_before;
                            int done = 5;
                        }
                        else
                        {
                    ...
                    keeping_the_same_code_as_before:
                    // else
                    //  {} do nothing
                }
                ...
                else
                {
                    ...
                    // if the user presses N , the original data on the database must be print to the file
                    pdt.product_code = original_temp_code;
                    pdt.product_price = original_temp_price;
                    pdt.product_quantity = original_temp_total_quantity;
                    strcpy(pdt.product_name, original_temp_name);
                    strcpy(pdt.product_company, original_temp_company_name);
...
// }
    ...
    inputf = fopen(fname, "a+");
        ...
        fprintf(inputf,"                    11/07/2022        \n"); // pass the date through the from time .h if you want to print the exact date or simply remove the date                             // fprintf("\t\t\t\t\t%s", ctime(&t)); // Print Time    
        fprintf(inputf,"\n");
        fprintf(inputf,"---------------------------------------\n");
        ...
        printf("                     - 12 June        \n");                              // fprintf("\t\t\t\t\t%s", ctime(&t)); // Print Time    pass the date through the from time .h if you want to print the exact date or simply remove the date
