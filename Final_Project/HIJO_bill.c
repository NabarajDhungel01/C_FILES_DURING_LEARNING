...
void add_product();
void delete_product();
void delete_productess(); // not working
void search_product();
...
// WE USE THIS VALUES TO STORE THE VALUE OF THE SELECTED CODE NUMBER TEMPORARAILY AND TO PRINT ON THE BILL
    ...
    {             // will be easy to minimize this portion so putting in if and make it always run
        ...
        printf("----------------------------------------------------------------------------------------");
        //  gotopositionxy(18,28);
        //  printf("%i\t %i\t%s\t\t  %i\t\t%s\t\t%i\t%i\t%i",sn,code,name,qt_left, company,qty,rate,total);
        // ↑↑↑↑↑↑↑↑↑↑↑↑↑↑ TEMPLATE OF BILL ↑↑↑↑↑↑↑↑↑↑↑↑↑↑
    }
...
ADD_PRODUCT_AGAIN:
    gotopositionxy(19,7); // this will show the cursor after the colon of code of product
    FILE *fp1;
    FILE *fp2;
    ...
    gotopositionxy(19,7);
    scanf("%d",&code);
    // CHECKS IF THE CODE IS PRESENT OR NOT and prints found if present   ↓↓↓↓↓
    ...
    while (fscanf(fp1, "%i %s %s %i %i\n", &pdt.product_code, pdt.product_name, pdt.product_company, &pdt.product_price, &pdt.product_quantity) != EOF)
        {
            if (code == pdt.product_code)
            {
                ...
                // dont multiply here and total some error will occur that will print the address of the temp_total _price instead of the value // temp_total_price = temp_price * qt ;
            }
        }
    //////////////////////////////////////////////////////
    // if i dont close here and open it again, the fscanf function will be pointing at the EOF and 
    // next time i use the while eof, it is already EOF, so to get the focus to the beginning 
    // i closed and open the file simply.

    // i dont know if there is function like seeking the beginning it would work too
    // i was coding offline with no internet access. so did'nt know abt it
    ...
    if(found = 1) // it continues no matter what bcz i have put only one equals = 1 is true 
    {
    ...
    getch();
    // printing the grand total price;
    ...
    printf("----------------------");
    printf("\n Press any key to print the customer's bill");
    getch();
...
// products and asl the user to choose the code number
// or there might be code 