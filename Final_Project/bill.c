...
//textcolor(8);
int sn, code, qt_left, qty, rate, total;
sn=1;
code=1;
qt_left =11;
rate=111;
...
// TO PRINT THE FRAME FOF THE BILL
    ...
    int found = 0 ; // initialzing with zero
    FILE *filep1;
    filep1 = fopen("ProductDetails.dat","r+"); // we are only reading this file
    FILE *filep2;
    filep2 = fopen("Temp_ProductDetails_Bills.dat","a+");

    // will display the details of entered code number for user to make it clear for user to verify the product and see the price and stock
        // while (fscanf(filep1,"%i %s %s %i %i\n", &pdt.product_code, pdt.product_name, pdt.product_company, &pdt.product_price, &pdt.product_quantity) != EOF)
            // {
    ...
    if (found == 1)
    {
     ...
     fflush(stdin);
        ...
        printf("\nConfirm -- || Code = %d ||  ||Quantity = %d|| ",code, qt);
        int total;
        ...
        if(check == 'y' || check == 'Y') 
        {
         ...
         while (fscanf(filep1,"%i %s %s %i %i\n", &pdt.product_code, pdt.product_name, pdt.product_company, &pdt.product_price, &pdt.product_quantity) != EOF)
         {
            if (code == pdt.product_code)
            {
                ...
                printf("\n\n\nQTY LEFT IS = %d",pdt.product_quantity);
                printf("\n\n\n LESS VAYO HAI, press enter");
                
                getch();
            }
                printf("\n\n\n ABT TO PRINT, press enter");
                getch();
                fprintf(filep2,"%i %s %s %i %i\n", pdt.product_code, pdt.product_name, pdt.product_company, pdt.product_price, pdt.product_quantity);
